...
웹 검증: JS 핵심 로직 10개 자동 시험(분리된 후보, 누락 모델, 손상 데이터, 44.1/48kHz 클릭 위치, 반복 시계, 경계·개수), 읽기 전용 API 통합 점검, 브라우저에서 재생/정지·후보 전환·120초 탐색·5초 확대·2~4초 반복을 확인했다. 이는 음악적 정확도를 귀로 평가해 합격했다는 뜻이 아니다.
