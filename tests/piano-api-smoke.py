...
def launch(folder):
    ...
    child = subprocess.Popen(['dotnet', str(ROOT / 'bin/Debug/net10.0/HueBeatController.dll'),
...
with tempfile.TemporaryDirectory(prefix='hue-piano-test-') as folder:
    child = launch(folder)
    try:
        assert request('/api/piano/config')[1] == {'enabled': False, 'sound': True, 'active': False}
        assert request('/api/piano/session', 'POST', {})[0] == 409
        a, b = 'aaaaaaaa-0000-0000-0000-000000000001', 'bbbbbbbb-0000-0000-0000-000000000002'
        settings = {'enabled': False, 'sound': False, 'assignments': [{'lightId': a, 'note': 3}, {'lightId': b, 'note': 3}], 'configurationIds': {}}
