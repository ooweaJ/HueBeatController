...
function drawAnalyzedTimeline(currentTime=0){
  const canvas=$('#meter'),ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);if(!analyzedTrack)return;
  ...
  const x=Math.max(0,Math.min(canvas.width,(currentTime/duration)*canvas.width));ctx.fillStyle='#ffffff';ctx.fillRect(x,0,2,canvas.height);$('#analysisVersion').textContent=`분석 v${analyzedTrack.version||'?'} · 타격 ${analyzedTrack.beatTimes?.length||0}개`;
}
