...
let ambientTimer=null,ambientStartedAt=0,ambientFrameBusy=false,ambientStreamActive=false,ambientOwner=null,ambientPlan=[],ambientPreviewSignature='';
...
function applyStoredControls(controls={}){
  ...
  if(controls.offlineShowWave!==undefined)$('#offlineShowWave').checked=controls.offlineShowWave===true;
  const ambient=controls.ambient||{};for(const [id,value] of Object.entries({ambientTarget:ambient.target,ambientEffect:ambient.effect,ambientSyncMode:ambient.syncMode,ambientDirection:ambient.direction,ambientCycle:ambient.cycle,ambientMin:ambient.min,ambientMax:ambient.max})){const input=document.getElementById(id);if(input&&value!==undefined){input.value=value;input.dispatchEvent(new Event('input'));}}
  if(ambient.colorMove!==undefined)$('#ambientColorMove').checked=Boolean(ambient.colorMove);if(ambient.previewOnly!==undefined)$('#ambientPreviewOnly').checked=Boolean(ambient.previewOnly);
...
document.querySelectorAll('[data-mode]').forEach(button => button.addEventListener('click', () => {
  if(button.dataset.mode==='music'&&(ambientTimer||ambientStreamActive))stopAmbient(false,false).catch(error=>setMessage(error.message,'error'));
...
function orderedLightRow(light, mode, groupId, index, count) {
  ...
  return `<div class="ordered-light-row draggable-chip ${mode==='music'?'with-piano-note':''}" draggable="${mode==='normal'}" data-drag-light="${light.id}" data-drag-mode="${mode}" data-light-slot="${light.id}" title="${String(index+1).padStart(2,'0')}번 · 배치 확인 시 ${colorName}">
    ${mode==='normal'?'<span class="light-drag-handle" aria-hidden="true">⠿</span>':''}<strong class="light-order-number">${String(index+1).padStart(2,'0')}</strong><span class="placement-swatch" style="background:${color}"></span><span class="ordered-light-name">${escapeHtml(light.name)} · B${light.bridgeIndex||1}</span>
    <span class="light-order-actions"><button type="button" data-rename-light="${light.id}" class="rename-order-light" aria-label="${escapeHtml(light.name)} 이름 수정" title="이름 수정">✎</button>${mode==='normal'?`<button type="button" data-light-order-offset="-1" ${index===0?'disabled':''} aria-label="${escapeHtml(light.name)} 앞으로 이동">↑</button><button type="button" data-light-order-offset="1" ${index===count-1?'disabled':''} aria-label="${escapeHtml(light.name)} 뒤로 이동">↓</button><button type="button" data-remove-light="${light.id}" data-remove-mode="${mode}" data-remove-group="${groupId}" aria-label="${escapeHtml(light.name)} 그룹에서 제거">×</button>`:''}</span>
...
function renderGroupManager(mode) {
  ...
  pool.innerHTML = `<div><strong>미배정 전구</strong><span>${mode==='music'?'Bridge 배치를 수정하면 음악 A/B에 자동 반영됩니다.':`${unassigned.length}개 · 그룹 카드로 드래그하거나 아래 선택 메뉴로 이동하세요.`}</span></div><div class="pool-chips">${unassigned.length ? unassigned.map(light => mode==='music'?`<span class="light-chip">${escapeHtml(light.name)} · B${light.bridgeIndex||1}</span>`:lightChip(light,false,mode)).join('') : '<small>모든 전구가 그룹에 배정되었습니다.</small>'}</div>`;
  ...
  container.innerHTML = groups.length ? groups.map((group,index) => {
    ...
    return `<article class="control-group-card" data-group-id="${group.id}" data-group-mode="${mode}" style="--group-color:${group.color}">
      ...
      <div class="assigned-lights drop-target">${members.length ? members.map((light,lightIndex) => orderedLightRow(light,mode,group.id,lightIndex,members.length)).join('') : `<small>${mode==='music'?'Bridge에 전구가 등록되면 자동으로 표시됩니다.':'전구를 이곳으로 드래그하세요.'}</small>`}</div>
      ${mode==='normal'?`<div class="group-move-row"><select data-group-light-select ${candidates.length ? '' : 'disabled'}>${candidates.map(light => `<option value="${light.id}">${escapeHtml(light.name)} · B${light.bridgeIndex||1}${groupForLight(mode,light.id) ? ` · ${escapeHtml(groupForLight(mode,light.id).name)}` : ' · 미배정'}</option>`).join('')}</select><button data-move-selected ${candidates.length ? '' : 'disabled'}>여기로 이동</button></div>`:''}
...
function orderedBridgeLights(bridgeIndex){
  ...
  const byId=new Map(members.map(light=>[light.id,light])),known=bridgeLightOrders[bridgeIndex].filter(id=>byId.has(id)),knownSet=new Set(known),added=members.filter(light=>!knownSet.has(light.id)).sort((left,right)=>left.name.localeCompare(right.name,'ko',{numeric:true})).map(light=>light.id),ids=[...known,...added];
  if(ids.join('|')!==bridgeLightOrders[bridgeIndex].join('|')){bridgeLightOrders[bridgeIndex]=ids;saveBridgeLightOrders();}
  return ids.map(id=>byId.get(id)).filter(Boolean);
}
function syncMusicGroupsToBridgeLayout(){
  const next=window.HueBridgeLayout.reconcileMusicGroups(groupState.music,bridgeLightOrders,lights);
  let changed=false;
  next.forEach((ids,index)=>{
    if(ids.join('|')===groupState.music[index].lightIds.join('|'))return;
    groupState.music[index].lightIds=ids;changed=true;
  });
...
function moveBridgeLightByOffset(bridgeIndex,lightId,offset){
  ...
  [ids[from],ids[to]]=[ids[to],ids[from]];bridgeLightOrders[bridgeIndex]=ids;saveBridgeLightOrders();syncMusicGroupsToBridgeLayout();renderLights();renderAllGroups();
}
function reorderBridgeLight(bridgeIndex,lightId,targetLightId){
  ...
  ids.splice(from,1);const adjusted=ids.indexOf(targetLightId)+(from<target?1:0);ids.splice(adjusted,0,lightId);bridgeLightOrders[bridgeIndex]=ids;saveBridgeLightOrders();syncMusicGroupsToBridgeLayout();renderLights();renderAllGroups();
}
function renderBridgeTransferManager(){
  ...
  const summary=$('#bridgeLayoutSummary');
  if(summary)summary.textContent=`현재 B1 ${counts[0]||0}개 → 음악 A · B2 ${counts[1]||0}개 → 음악 B. 위 순서가 음악 배치 색상과 연출 순서에 자동 반영됩니다.`;
...
async function loadLights(quiet=false) {
  ...
  try{lights=await api('/api/lights');orderedBridgeLights(1);orderedBridgeLights(2);syncMusicGroupsToBridgeLayout();renderLights();renderAllGroups();refreshAmbientPreview();if(!quiet){const connected=lights.filter(light=>light.connectivity==='connected').length,counts=[1,2].map(index=>lights.filter(light=>light.bridgeIndex===index).length);setMessage(`Bridge 1 ${counts[0]}개 + Bridge 2 ${counts[1]}개 · 총 ${lights.length}개 중 ${connected}개 연결`,connected?'success':'');}}
  catch(error){setMessage(error.message,'error');}finally{button.disabled=false;}
}
...
function ambientOrderedIds(bridgeIndex,allowedIds){const allowed=new Set((allowedIds||[]).map(String)),ordered=bridgeLightOrders[bridgeIndex].filter(id=>allowed.has(id)),seen=new Set(ordered);return [...ordered,...[...allowed].filter(id=>!seen.has(id))];}
function ambientSettings(){return{effect:$('#ambientEffect').value,syncMode:$('#ambientSyncMode').value,direction:$('#ambientDirection').value,cycleSec:Number($('#ambientCycle').value),minBrightness:Number($('#ambientMin').value),maxBrightness:Number($('#ambientMax').value),colorMove:$('#ambientColorMove').checked};}
function buildAmbientPlan(requireConfiguration){
  return ambientTargetBridgeIndexes().map(bridgeIndex=>{
    const paired=bridgeStatuses.some(item=>item.bridgeIndex===bridgeIndex&&item.paired),configuration=ambientConfiguration(bridgeIndex);
    if(requireConfiguration&&!paired)throw new Error(`Bridge ${bridgeIndex}을 먼저 인증하세요.`);
    if(requireConfiguration&&!configuration)throw new Error(`Bridge ${bridgeIndex}의 Entertainment 영역을 음악 모드에서 선택하세요.`);
    const available=configuration?.lightIds?.length?configuration.lightIds:bridgeLightOrders[bridgeIndex].length?bridgeLightOrders[bridgeIndex]:lights.filter(light=>Number(light.bridgeIndex||1)===bridgeIndex).map(light=>light.id);
    let lightIds=ambientOrderedIds(bridgeIndex,available);
    if(requireConfiguration&&!lightIds.length)throw new Error(`Bridge ${bridgeIndex}의 Entertainment 영역에 전구가 없습니다.`);
    if(!requireConfiguration&&!lightIds.length)lightIds=Array.from({length:8},(_,index)=>`preview-${bridgeIndex}-${index+1}`);
    return{bridgeIndex,configurationId:configuration?.id||'',lightIds};
  });
}
function ambientFrameAt(elapsedSec,plan=ambientPlan){return HueAmbientArt.buildFrame({...ambientSettings(),elapsedSec,bridges:plan.map(item=>({bridgeIndex:item.bridgeIndex,lightIds:item.lightIds}))});}
function ensureAmbientPreview(plan){
  const signature=plan.map(item=>`${item.bridgeIndex}:${item.lightIds.join(',')}`).join('|');if(signature===ambientPreviewSignature)return;ambientPreviewSignature=signature;
  const root=$('#ambientPreview');root.innerHTML=plan.map(item=>`<div class="ambient-preview-row" style="--ambient-count:${item.lightIds.length}"><strong>Bridge ${item.bridgeIndex}</strong>${item.lightIds.map((id,index)=>`<div class="ambient-preview-light"><i class="ambient-preview-bulb" data-ambient-id="${escapeHtml(id)}"></i><small>${escapeHtml(lights.find(light=>light.id===id)?.name||`${item.bridgeIndex}-${index+1}`)}</small></div>`).join('')}</div>`).join('');
}
function renderAmbientPreview(frame,plan=ambientPlan){
  ensureAmbientPreview(plan);const byId=new Map(frame.lights.map(item=>[item.id,item]));document.querySelectorAll('[data-ambient-id]').forEach(bulb=>{const light=byId.get(bulb.dataset.ambientId),rgb=hexToRgb(light?.color||'#000000'),level=Math.max(0,Math.min(1,(light?.brightness||0)/100));bulb.style.setProperty('--ambient-rgb',`${rgb.r},${rgb.g},${rgb.b}`);bulb.style.setProperty('--ambient-opacity',String(.16+level*.84));bulb.style.setProperty('--ambient-scale',String(.9+level*.1));bulb.style.setProperty('--ambient-glow',`${4+level*28}px`);bulb.style.setProperty('--ambient-glow-alpha',String(level*.78));});
}
function refreshAmbientPreview(){if(ambientTimer)return;ambientPlan=buildAmbientPlan(false);ambientPreviewSignature='';renderAmbientPreview(ambientFrameAt(0,ambientPlan),ambientPlan);}
function setAmbientUi(running,text,ready=false){$('#ambientStartButton').disabled=running;$('#ambientStopButton').disabled=!running;const status=$('#ambientStatus');status.textContent=text;status.className=`analysis-state${ready?' ready':''}`;}
function cancelAmbientLoop(silent=true){if(ambientTimer){clearInterval(ambientTimer);ambientTimer=null;}ambientFrameBusy=false;if(!silent)setAmbientUi(false,'정지됨');}
async function sendAmbientFrame(frame){
  if(!entertainmentActive||ambientFrameBusy)return false;ambientFrameBusy=true;
  try{const commands=frame.lights.map(light=>({lightIds:[light.id],hexColor:light.color,brightness:light.brightness,transitionMs:80,on:true,groupKey:`ambient-${light.bridgeIndex}`})),result=await api('/api/entertainment/frame',{method:'POST',body:JSON.stringify({commands,scheduleAheadMs:ENTERTAINMENT_SCHEDULE_AHEAD_MS,owner:ambientOwner})});if(result.ignoredLightIds?.length)throw new Error(`${result.ignoredLightIds.length}개 전구가 선택한 Entertainment 영역에 없습니다.`);return true;}finally{ambientFrameBusy=false;}
}
function tickAmbient(){
  if(!ambientTimer)return;const elapsed=(performance.now()-ambientStartedAt)/1000,frame=ambientFrameAt(elapsed);renderAmbientPreview(frame);if(!ambientStreamActive)return;
  sendAmbientFrame(frame).catch(error=>{const owner=ambientOwner;cancelAmbientLoop(true);ambientStreamActive=false;ambientOwner=null;entertainmentActive=false;$('#startEntertainmentButton').disabled=false;$('#stopEntertainmentButton').disabled=true;setAmbientUi(false,'연결 변경');if(owner)api('/api/entertainment/stop-ambient',{method:'POST',body:JSON.stringify({owner})}).catch(()=>{});setMessage(`상시 연출 연결이 종료됐습니다: ${error.message}`,'error');});
}
async function startAmbient(){
  $('#offlineShowAudio').pause();await releaseOfflineShowStream();stopCalibration(true);stopShowTest(true);const player=$('#audioPlayer');player.pause();stopPlaybackLoop();cancelAmbientLoop(true);if(entertainmentActive)await stopEntertainment(true);
  const previewOnly=$('#ambientPreviewOnly').checked;if(!previewOnly&&!entertainmentConfigurations.length)await loadEntertainmentConfigurations();ambientPlan=buildAmbientPlan(!previewOnly);
  if(!previewOnly){const bridges=ambientPlan.map(item=>({bridgeIndex:item.bridgeIndex,configurationId:item.configurationId})),result=await api('/api/entertainment/start',{method:'POST',body:JSON.stringify({bridges,purpose:'ambient'})});entertainmentActive=true;ambientStreamActive=true;ambientOwner=result.owner;$('#startEntertainmentButton').disabled=true;$('#stopEntertainmentButton').disabled=false;$('#entertainmentStatus').className='analysis-state ready';$('#entertainmentStatus').textContent=`상시 연출 연결됨 · Bridge ${result.activeBridges||bridges.length}대`;}
  ambientStartedAt=performance.now();setAmbientUi(true,previewOnly?'웹 미리보기 실행 중':`실제 전구 실행 중 · ${ambientPlan.map(item=>`B${item.bridgeIndex} ${item.lightIds.length}개`).join(' / ')}`,true);ambientTimer=setInterval(tickAmbient,ENTERTAINMENT_FRAME_INTERVAL_MS);tickAmbient();queueControllerSettingsSave();
}
async function stopAmbient(silent=false,turnOff=false){
  const plan=ambientPlan.length?ambientPlan:buildAmbientPlan(false),owner=ambientOwner,wasStreaming=ambientStreamActive&&entertainmentActive;cancelAmbientLoop(true);
  try{if(turnOff&&wasStreaming){const commands=plan.flatMap(item=>item.lightIds.map(id=>({lightIds:[id],hexColor:null,brightness:0,transitionMs:120,on:false})));await api('/api/entertainment/frame',{method:'POST',body:JSON.stringify({commands,scheduleAheadMs:0,owner})});renderAmbientPreview({lights:plan.flatMap(item=>item.lightIds.map(id=>({id,color:'#000000',brightness:0})))},plan);}}
  finally{if(wasStreaming&&owner)await api('/api/entertainment/stop-ambient',{method:'POST',body:JSON.stringify({owner})}).catch(()=>{});ambientOwner=null;ambientStreamActive=false;entertainmentActive=false;$('#startEntertainmentButton').disabled=false;$('#stopEntertainmentButton').disabled=true;$('#entertainmentStatus').className='analysis-state';$('#entertainmentStatus').textContent='연결 안 됨';setAmbientUi(false,turnOff?'선택 대상 소등':'정지됨');}
  if(!silent)setMessage(turnOff?'상시 연출을 정지하고 선택한 전구를 껐습니다.':'상시 연출을 정지했습니다.','success');
}
async function turnAmbientOff(){
  if(ambientTimer||ambientStreamActive){await stopAmbient(false,true);return;}
  const previewOnly=$('#ambientPreviewOnly').checked,plan=buildAmbientPlan(false),dark={lights:plan.flatMap(item=>item.lightIds.map(id=>({id,color:'#000000',brightness:0})))};renderAmbientPreview(dark,plan);
  if(previewOnly){setAmbientUi(false,'미리보기 소등');setMessage('웹 미리보기만 껐습니다. 실제 전구에는 명령을 보내지 않았습니다.','success');return;}
  const ids=plan.flatMap(item=>item.lightIds).filter(id=>connectedLightIds().includes(id));if(!ids.length)throw new Error('선택한 Bridge에 연결된 전구가 없습니다.');await sendCommands([{lightIds:ids,hexColor:null,brightness:0,transitionMs:120,on:false}]);setAmbientUi(false,'선택 대상 소등');setMessage('선택한 Bridge의 전구를 껐습니다.','success');
}
...
async function stopEntertainment(silent=false){
  cancelAmbientLoop(true);try{await api('/api/entertainment/stop',{method:'POST'});}catch(error){if(!silent)throw error;}finally{entertainmentActive=false;ambientStreamActive=false;ambientOwner=null;entertainmentFrameBusy=false;$('#startEntertainmentButton').disabled=false;$('#stopEntertainmentButton').disabled=true;$('#entertainmentStatus').className='analysis-state';$('#entertainmentStatus').textContent='연결 안 됨';if($('#ambientStartButton'))setAmbientUi(false,'정지됨');}
}
...
async function startShowTest(){
  $('#offlineShowAudio').pause();await releaseOfflineShowStream();
  if(ambientTimer||ambientStreamActive)await stopAmbient(true,false);
...
function setMusicStyle(style){
  ...
  $('#equalizerGroupField').hidden=true;$('#entertainmentControls').hidden=false;$('#musicStyleDescription').textContent='새 사전 분석 연출은 저장된 마디 첫 박자·클라이맥스 구간을 사용합니다. 아래의 40초 반복 테스트와 이전 저장 음원은 별도 실험 경로입니다.';updateEntertainmentMapping();if(!entertainmentConfigurations.length)loadEntertainmentConfigurations().catch(error=>setMessage(error.message,'error'));
}
...
$('#startEntertainmentButton').addEventListener('click',async()=>{try{$('#offlineShowAudio').pause();await releaseOfflineShowStream();const result=await startEntertainment();setMessage(result.message,'success');}catch(error){setMessage(error.message,'error');}});
$('#stopEntertainmentButton').addEventListener('click',async()=>{try{$('#offlineShowAudio').pause();await releaseOfflineShowStream();await stopEntertainment(false);setMessage('Entertainment 스트리밍을 종료했습니다.');}catch(error){setMessage(error.message,'error');}});
$('#ambientStartButton').addEventListener('click',()=>startAmbient().catch(error=>{setAmbientUi(false,'시작 실패');setMessage(`상시 연출을 시작하지 못했습니다: ${error.message}`,'error');}));
$('#ambientStopButton').addEventListener('click',()=>stopAmbient(false,false).catch(error=>setMessage(error.message,'error')));
$('#ambientOffButton').addEventListener('click',()=>turnAmbientOff().catch(error=>setMessage(error.message,'error')));
['ambientEffect','ambientSyncMode','ambientDirection','ambientColorMove'].forEach(id=>document.getElementById(id).addEventListener('change',()=>{queueControllerSettingsSave();refreshAmbientPreview();}));
['ambientTarget','ambientPreviewOnly'].forEach(id=>document.getElementById(id).addEventListener('change',()=>{const restart=Boolean(ambientTimer||ambientStreamActive);if(restart)stopAmbient(true,false).catch(error=>setMessage(error.message,'error'));queueControllerSettingsSave();refreshAmbientPreview();if(restart)setMessage('실행 대상 또는 출력 방식을 바꿨습니다. 상시 연출을 다시 시작하세요.','success');}));
...
bindRange('#manualTransition','#manualTransitionValue',' ms');bindRange('#beatBrightness','#beatBrightnessValue','%');bindRange('#transition','#transitionValue',' ms');bindRange('#testBpm','#testBpmValue',' BPM');bindRange('#sensitivity','#sensitivityValue');bindRange('#minInterval','#intervalValue',' ms');bindRange('#syncOffset','#syncOffsetValue',' ms 빠르게');bindRange('#entertainmentAccentInterval','#entertainmentAccentIntervalValue',' ms');bindRange('#entertainmentPunch','#entertainmentPunchValue','%');bindRange('#ambientCycle','#ambientCycleValue','초');bindRange('#ambientMin','#ambientMinValue','%');bindRange('#ambientMax','#ambientMaxValue','%');
['manualTransition','beatBrightness','transition','testBpm','syncOffset','sensitivity','minInterval','entertainmentAccentInterval','entertainmentPunch','ambientCycle','ambientMin','ambientMax'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>{if(id==='ambientMin'&&Number($('#ambientMin').value)>Number($('#ambientMax').value)){$('#ambientMax').value=$('#ambientMin').value;$('#ambientMax').dispatchEvent(new Event('input'));}if(id==='ambientMax'&&Number($('#ambientMax').value)<Number($('#ambientMin').value)){$('#ambientMin').value=$('#ambientMax').value;$('#ambientMin').dispatchEvent(new Event('input'));}queueControllerSettingsSave();if(id.startsWith('ambient'))refreshAmbientPreview();}));
...
async function startSyncVerification(){
  if(ambientTimer||ambientStreamActive)await stopAmbient(true,false);stopCalibration(true);stopShowTest(true);const player=$('#audioPlayer');player.pause();resetAnalysis();try{await ensureAudio();validateEntertainmentGroups();if(!entertainmentActive)await startEntertainment();}catch(error){setMessage(`통합 테스트를 시작하지 못했습니다: ${error.message}`,'error');return;}syncVerificationActive=true;setCalibrationControls(true);$('#calibrationStatus').textContent='통합 싱크 테스트를 준비합니다.';calibrationTimer=setTimeout(scheduleSyncVerification,400);
}
...
async function startAnalyzedPlayback(){
  if(!analyzedTrack)return;$('#offlineShowAudio').pause();await releaseOfflineShowStream();if(ambientTimer||ambientStreamActive)await stopAmbient(true,false);const player=$('#audioPlayer'),previewOnly=$('#analysisPreviewOnly').checked;stopCalibration(true);if(!previewOnly&&musicStyle==='beat'&&!musicScenesReady){player.pause();try{await prepareMusicScenes();await player.play();}catch(error){setMessage(`공통 Scene 준비 실패: ${error.message}`,'error');}return;}if(!previewOnly&&musicStyle==='beat-brightness'&&!musicGroupsReady){player.pause();try{await prepareAnalyzedMusicGroups();await player.play();}catch(error){setMessage(`음악 그룹 준비 실패: ${error.message}`,'error');}return;}if(!previewOnly&&musicStyle==='entertainment'&&!entertainmentActive){player.pause();try{await startEntertainment();await player.play();}catch(error){setMessage(`Entertainment 준비 실패: ${error.message}`,'error');}return;}stopShowTest(true);if(activeStream){activeStream.getTracks().forEach(track=>track.stop());activeStream=null;}resetAnalyzedPlayback(player.currentTime);stopPlaybackLoop();if(previewOnly)setMessage('분석 미리보기 재생 중 · Hue Bridge와 전구에는 명령을 보내지 않습니다.','success');
...
function offlineShowStatus(message,error=false){const node=$('#offlineShowStatus');node.textContent=message;node.className=`analysis-state${error?' error':''}`;}
function offlineShowBase(projectId,revisionId){return `/api/offline-review/projects/${encodeURIComponent(projectId)}/analyses/${encodeURIComponent(revisionId)}`;}
function populateOfflineShowRevisions(preferred=''){
  const project=offlineShowProjects.find(item=>item.projectId===$('#offlineShowProject').value),select=$('#offlineShowRevision');
  select.innerHTML=(project?.revisions||[]).map(item=>`<option value="${escapeHtml(item.analysisId)}">${escapeHtml(new Date(item.createdAt).toLocaleString('ko-KR'))} · ${escapeHtml(item.analysisId.slice(0,8))}</option>`).join('')||'<option value="">분석 버전 없음</option>';
  if(preferred&&project?.revisions.some(item=>item.analysisId===preferred))select.value=preferred;
}
async function refreshOfflineShowProjects(){
  const previousProject=$('#offlineShowProject').value,previousRevision=$('#offlineShowRevision').value;
  offlineShowProjects=await api('/api/offline-review/projects');
  $('#offlineShowProject').innerHTML=offlineShowProjects.map(item=>`<option value="${escapeHtml(item.projectId)}">${escapeHtml(item.title)} · ${Math.round(item.durationSec)}초</option>`).join('')||'<option value="">분석한 음원 없음</option>';
  if(offlineShowProjects.some(item=>item.projectId===previousProject))$('#offlineShowProject').value=previousProject;
  populateOfflineShowRevisions(previousRevision);
...
function renderOfflineShowFrame(frame,time){
  const pairs=frame.a.length,commands=offlineShowCommands(frame,false);ensureVirtualLights(pairs);
  document.querySelectorAll('[data-virtual-slot]').forEach(bulb=>{
    const row=bulb.dataset.virtualRow==='B'?1:0,index=Number(bulb.dataset.virtualSlot),command=commands[row*pairs+index],rgb=hexToRgb(command.hexColor),level=command.brightness/100;
    bulb.style.setProperty('--bulb-color',command.hexColor);bulb.style.setProperty('--bulb-rgb',`${rgb.r},${rgb.g},${rgb.b}`);
    bulb.style.setProperty('--bulb-shine',(.08+level*.55).toFixed(3));bulb.style.setProperty('--bulb-alpha',(.16+level*.84).toFixed(3));bulb.style.setProperty('--bulb-soft',(.05+level*.48).toFixed(3));bulb.style.setProperty('--bulb-glow',`${5+level*34}px`);bulb.style.setProperty('--bulb-glow-alpha',(level*.82).toFixed(3));bulb.style.setProperty('--bulb-inset',`${5+level*15}px`);bulb.style.setProperty('--bulb-inset-alpha',(level*.32).toFixed(3));bulb.style.setProperty('--bulb-scale',(.93+level*.07).toFixed(3));
    bulb.classList.toggle('hit',frame.accumulation?.phase==='punch'||frame.mode==='climax'&&level>.9);
    bulb.parentElement.querySelector('small').textContent=`${bulb.dataset.virtualRow}${index+1} · ${Math.round(command.brightness)}%`;
  });
  $('#simulatorState').textContent=`${frame.mode==='climax'?'클라이맥스':frame.mode==='buildup'?'진입 준비':'일반'} · ${frame.pattern||'점등'}`;
  $('#simulatorState').classList.toggle('climax',frame.mode==='climax');
  $('#simulatorTime').textContent=`${Math.max(0,time).toFixed(2)}s`;
  $('#simulatorLow').textContent=`${Math.round((frame.energy||0)*100)}%`;$('#simulatorMid').textContent='—';$('#simulatorHigh').textContent='—';
}
async function sendOfflineShowFrame(frame){
  if(!offlineShowStreamOwned||!entertainmentActive||offlineShowSendBusy)return;
  offlineShowSendBusy=true;
  try{const result=await api('/api/entertainment/frame',{method:'POST',body:JSON.stringify({commands:offlineShowCommands(frame,true),scheduleAheadMs:ENTERTAINMENT_SCHEDULE_AHEAD_MS})});if(result.ignoredLightIds?.length)throw new Error(`${result.ignoredLightIds.length}개 전구가 선택한 Entertainment 영역에 없습니다.`);}
  finally{offlineShowSendBusy=false;}
}
function stopOfflineShowLoop(){if(offlineShowTimer){clearTimeout(offlineShowTimer);offlineShowTimer=null;}}
async function releaseOfflineShowStream(){
  if(offlineShowReleasePromise)return offlineShowReleasePromise;
  if(!offlineShowStreamOwned)return;
  offlineShowReleasePromise=(async()=>{
...
function tickOfflineShow(){
  const player=$('#offlineShowAudio');if(!offlineShowSession||player.paused||player.ended){stopOfflineShowLoop();return;}
  try{const frame=offlineShowFrame(player.currentTime);renderOfflineShowFrame(frame,player.currentTime);if(offlineShowStreamOwned&&!offlineShowSendBusy){const pending=sendOfflineShowFrame(frame);offlineShowPendingFrame=pending;pending.catch(error=>{offlineShowStatus(`전구 전송 실패: ${error.message}`,true);player.pause();}).finally(()=>{if(offlineShowPendingFrame===pending)offlineShowPendingFrame=null;});}}
  catch(error){offlineShowStatus(error.message,true);player.pause();return;}
  offlineShowTimer=setTimeout(tickOfflineShow,ENTERTAINMENT_FRAME_INTERVAL_MS);
}
async function loadOfflineShow(){
  const projectId=$('#offlineShowProject').value,revisionId=$('#offlineShowRevision').value,token=++offlineShowLoadToken,player=$('#offlineShowAudio');
  player.pause();stopOfflineShowLoop();await releaseOfflineShowStream();offlineShowSession=null;player.removeAttribute('src');player.load();
  if(!projectId||!revisionId)throw new Error('분석한 음원과 버전을 선택하세요.');
  offlineShowStatus('분석 데이터와 저장된 연출을 불러오는 중…');
  const base=offlineShowBase(projectId,revisionId),[analysis,show]=await Promise.all([api(base),api(`${base}/show`)]);
  if(token!==offlineShowLoadToken)return;
  offlineShowSession=HueOfflineShowPlayback.prepare(analysis,show);
  player.src=`/api/offline-review/projects/${encodeURIComponent(projectId)}/audio`;player.load();
  renderOfflineShowFrame(offlineShowFrame(0),0);
  offlineShowStatus(`준비 완료 · 분석 ${revisionId.slice(0,8)} · 저장 연출 v${show.version} · 마디 첫 박자 ${offlineShowSession.events.length}개 · 재생을 누르세요.`);
}
async function beginOfflineShowPlayback(){
  const player=$('#offlineShowAudio');if(!offlineShowSession){player.pause();offlineShowStatus('연출 불러오기를 먼저 누르세요.',true);return;}
  if(offlineShowPreparing)return;
  ...
  if($('#offlineShowPreviewOnly').checked){$('#audioPlayer').pause();stopShowTest(true);if(entertainmentActive)await stopEntertainment(true);offlineShowStatus('웹 모의 전구 재생 중 · 실제 Hue에는 전송하지 않습니다.');}
  stopOfflineShowLoop();tickOfflineShow();
}
function invalidateOfflineShowSelection(){
  ++offlineShowLoadToken;const player=$('#offlineShowAudio');player.pause();stopOfflineShowLoop();offlineShowSession=null;player.removeAttribute('src');player.load();
  offlineShowStatus('선택한 분석 버전의 연출 불러오기를 누르세요.');
}
$('#offlineShowProject').addEventListener('change',()=>{populateOfflineShowRevisions();invalidateOfflineShowSelection();});
$('#offlineShowRevision').addEventListener('change',invalidateOfflineShowSelection);
$('#offlineShowRefresh').addEventListener('click',()=>refreshOfflineShowProjects().catch(error=>offlineShowStatus(error.message,true)));
$('#offlineShowLoad').addEventListener('click',()=>loadOfflineShow().catch(error=>offlineShowStatus(error.message,true)));
$('#offlineShowWave').addEventListener('change',()=>{queueControllerSettingsSave();if(offlineShowSession)renderOfflineShowFrame(offlineShowFrame($('#offlineShowAudio').currentTime),$('#offlineShowAudio').currentTime);});
$('#offlineShowPreviewOnly').addEventListener('change',()=>{$('#offlineShowAudio').pause();offlineShowStatus($('#offlineShowPreviewOnly').checked?'웹 모의 전구 모드 · 다시 재생하세요.':'실제 Hue 출력 모드 · 전구 연결 후 다시 재생하세요.');});
$('#offlineShowAudio').addEventListener('play',()=>beginOfflineShowPlayback().catch(error=>{offlineShowStatus(error.message,true);$('#offlineShowAudio').pause();}));
$('#offlineShowAudio').addEventListener('pause',()=>{stopOfflineShowLoop();if(!offlineShowPreparing)releaseOfflineShowStream().catch(error=>offlineShowStatus(error.message,true));});
$('#offlineShowAudio').addEventListener('seeked',()=>{if(offlineShowSession){const time=$('#offlineShowAudio').currentTime;renderOfflineShowFrame(offlineShowFrame(time),time);}});
$('#offlineShowAudio').addEventListener('ended',()=>{stopOfflineShowLoop();releaseOfflineShowStream().catch(error=>offlineShowStatus(error.message,true));offlineShowStatus('재생 완료 · Entertainment 스트림을 정지했습니다.');});
...
$('#audioPlayer').addEventListener('play',()=>{$('#offlineShowAudio').pause();renderTrackLibrary();startAnalyzedPlayback();});
