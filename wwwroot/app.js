...
async function analyzeAudioBuffer(buffer,onProgress=()=>{}){
  ...
  const result={version:8,duration:buffer.duration,bpm,beatInterval,beatGridStart,beatGridOffsetMs:0,beatGrid,downbeatPhase,downbeats,beatTimes,cueStrengths,rawBassHitTimes,envelope,bassEnvelope,bassOnsetEnvelope,onsetEnvelope,envelopeStep:.1,confidence:Math.max(0,Math.min(1,bestScore||0)),analysisMode:'beatgrid-bass-onset'};
...
function paintAnalyzedTimeline(canvas,currentTime=0,rangeStart=0,rangeEnd=null){
  ...
  if(span<=30)drawGrid(analyzedTrack.beatGrid,'#62a8ff',1,.24);drawGrid(analyzedTrack.downbeats,'#22e5ee',Math.max(1.5,canvas.width/1200*2),.48);
...
function editBeatgrid({bpmFactor=1,shiftMs=0,downbeatShift=0}){if(!analyzedTrack?.beatGrid?.length){setMessage('Beatgrid가 포함된 분석 v8 음원을 먼저 선택하세요.','error');return;}if(bpmFactor!==1)analyzedTrack.beatInterval=Math.max(.25,Math.min(1.5,analyzedTrack.beatInterval/bpmFactor));if(shiftMs){analyzedTrack.beatGridStart=Math.max(0,Math.min(analyzedTrack.duration-.01,analyzedTrack.beatGridStart+shiftMs/1000));analyzedTrack.beatGridOffsetMs=(Number(analyzedTrack.beatGridOffsetMs)||0)+shiftMs;}if(downbeatShift)analyzedTrack.downbeatPhase=(Number(analyzedTrack.downbeatPhase)||0)+downbeatShift;rebuildBeatGrid();drawAnalyzedTimeline($('#audioPlayer').currentTime||0);}
...
async function upgradeSavedTrackAnalysis(track){
  const slotCount=entertainmentPairCount();if(Number(track.analysis?.version||0)>=8&&track.analysis?.mediaArt?.version===5&&track.analysis.mediaArt.slotCount===slotCount)return track;
  if(Number(track.analysis?.version||0)>=8&&Array.isArray(track.analysis?.envelope)){
