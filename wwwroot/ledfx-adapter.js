// Original LedFx produces every colour. No beat detection or show rules here.
(() => {
  ...
  panel.innerHTML = `
    ...
    <div class="entertainment-row">
      ...
      <label>원본 효과<select id="lfEffect"><option value="energy">Energy — 대역별 에너지</option><option value="bar">Bar — 박자 이동</option><option value="power">Power — 배경·타격</option></select></label>
    ...
    <div id="lfBulbs"></div><small>원본 RGB 출력 · 같은 번호를 A/B에 복제 · 실시간 분석이라 기존 사전 분석 싱크값은 적용하지 않습니다.<br>Energy는 전구 수가 적거나 소리가 작으면 막대 길이가 0이 될 수 있습니다. 1쌍 테스트는 Power를 사용하세요. 빠른 점멸에 민감하면 실제 조명 출력을 켜지 마세요.</small>`;
  ...
  async function configure() {
    ...
    return api('/api/ledfx/configure',{method:'POST',body:JSON.stringify({pairs,effect:el('lfEffect').value,client:'HueBeat-Web'})});
  }
  ...
  async function poll(token) {
    ...
    try {
      ...
      if(frame.stale){draw(Array(Number(el('lfPairs').value)*3).fill(0));await queueOutput(releaseHue);status('원본 프레임 대기 중 · 음악 재생과 엔진 연결을 확인하세요.');}
      else if(frame.sequence!==sequence && frame.rgb.length===Number(el('lfPairs').value)*3){
        sequence=frame.sequence;draw(player.paused?frame.rgb.map(()=>0):frame.rgb);
        if(el('lfHue').checked&&!player.paused){
          await queueOutput(async()=>{
            ...
            const result=await api('/api/entertainment/frame',{method:'POST',body:JSON.stringify({commands:commands(frame.rgb,groups),scheduleAheadMs:0})});
            if(result.ignoredLightIds?.length)throw Error('선택한 Entertainment 영역에 없는 전구가 있습니다.');
          });
        }
        status('원본 '+el('lfEffect').value+' · RGB 프레임 '+sequence+(el('lfHue').checked?' · Hue 출력':' · 웹 미리보기'));
      }
  ...
  el('lfApply').onclick=run(async()=>{if(!ready)throw Error('먼저 엔진을 연결하세요.');await configure();sequence=-1;status('원본 효과 적용 완료');});
