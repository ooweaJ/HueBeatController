(function(root){
  ...
  function refresh(){
    ...
    for(const light of lights){
      ...
      const side=groups.findIndex(g=>g.lightIds.includes(light.id)),position=side<0?-1:groups[side].lightIds.indexOf(light.id);
      detail.textContent=`Bridge ${light.bridgeIndex} · ${side<0?'음악 그룹 미지정':`${side?'B':'A'}${position+1}`} · ${light.connectivity==='connected'?'연결됨':light.connectivity==='unknown'?'상태 미확인':'연결 끊김'}`;
      ...
      select.addEventListener('change',()=>{if(select.value==='')map.delete(light.id);else map.set(light.id,Number(select.value));changed();summary();});
    ...
    if(!lights.length&&!missing.length)container.textContent='Bridge 전구를 불러오면 전구마다 음계를 지정할 수 있습니다.';
  ...
  async function init(options){
    ...
    $('pianoCopyMusic').addEventListener('click',()=>{
      ...
      draft.configurationIds={...source.getSelectedConfigurations()};refresh();changed();
    });
    ...
    try{
      ...
      ready=true;$('pianoSave').disabled=false;$('pianoCopyMusic').disabled=false;refresh();status('전구 위치를 확인하고 각 전구의 음계를 지정하세요.');
    }catch(error){status(`피아노 설정을 불러오지 못했습니다: ${error.message}. 서버를 최신 빌드로 실행해 주세요.`,true);}
  }
  root.HuePianoOperator={init,refresh};
})(window);
